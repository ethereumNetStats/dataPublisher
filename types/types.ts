...
// データプールサーバーから受け取る"sendBlockList"イベントで受信するデータの型定義
type blockList = {
...
// blockListTypeからフロントエンドIDを削除した型定義（ブラウザではフロントエンドIDは使わないため）
type blockListTypeWithoutFrontendId = Omit<blockList, 'frontendId'>

// データプールサーバーから発行される"requestBlockListPageByBlockNumber"イベントで受け取るデータの型定義
type requestBlockListPageByBlockNumberType = {
  blockNumber: number
  frontendId: string
}
...
type resultOfBlockSearchWithoutFrontendId = Omit<resultOfBlockSearch, 'frontendId'>
