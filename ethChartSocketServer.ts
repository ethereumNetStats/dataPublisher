...
ethChartSocketClient.on('connect', () => {
    ...
    ethChartSocketClient.emit('requestInitialBlockData');
...
ethChartSocketClient.on('stillNoInitialBlockData', () => {
...
ethChartSocketClient.on('initialBlockData', (initialBlockData: blockDataArray) => {
...
ethChartSocketClient.on('newBlockData', (newBlockData: blockData) => {
    ...
    if (blockDataArray.length !== 0) {
        ...
        // 受け取った最新値をフロントエンドに転送
        ethChartSocketServer.emit('newBlockDataToFrontend', newBlockData);
...
ethChartSocketClient.on('responseBlockDetail', (responseBlockDetail: responseBlockDetail) => {
    ...
    ethChartSocketServer.to(responseBlockDetail.frontendId).emit('responseBlockDetail', responseBlockDetail);
...
ethChartSocketClient.on('responseBlockList', (responseBlockList: responseBlockList) => {
    console.log(`${currentTimeReadable()} | Receive : 'responseBlockList' | From : dataPoolServer`);
    // 要求したユーザーのフロントエンドに結果を転送
    ethChartSocketServer.to(responseBlockList.frontendId).emit(`responseBlockList`, responseBlockList);
...
ethChartSocketClient.on('responseBlockListPageByBlockNumber', (responseBlockListPageByBlockNumber: responseBlockListPageByBlockNumber) => {
    console.log(`${currentTimeReadable()} | Receive : 'responseBlockListPageByBlockNumber' | From : dataPoolServer`);
    // 検索を要求したユーザーのフロントエンドに結果を転送
    ethChartSocketServer.to(responseBlockListPageByBlockNumber.frontendId).emit('responseBlockListPageByBlockNumber', responseBlockListPageByBlockNumber);
...
ethChartSocketServer.on('connect', (frontend) => {
    ...
    frontend.on('requestBlockDetail', (number: number) => {
        ...
        ethChartSocketClient.emit('requestBlockDetail', ({number: number, frontendId: frontend.id}));
    ...
    frontend.on('requestBlockList', (pageOffset: number) => {
        ...
        ethChartSocketClient.emit('requestBlockList', ({pageOffset: pageOffset, frontendId: frontend.id}));
    ...
    frontend.on('requestBlockListPageByBlockNumber', (blockNumber: number) => {
        ...
        ethChartSocketClient.emit('requestBlockListPageByBlockNumber', {blockNumber: blockNumber, frontendId: frontend.id});
